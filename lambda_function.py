...
def parse_announcement(event_payload):
    ...
    if channel_id != os.environ.get("SLACK_CHANNEL_ID", "C067Z2CJ0H0"):
        return None
    ...
    properties = {
        ...
        "sourceChannel": os.environ.get("SLACK_CHANNEL_NAME", ""),
        "sourceChannelId": channel_id,
        "sourceMessageTs": ts,
        "rawMessage": text,
        "summary": summary[:120],
        "targetRepositoryUrl": os.environ.get(
            "TARGET_REPOSITORY_URL",
            "https://github.com/samdeepk-eng/Feature-releases.git",
        ),
        "targetBranch": os.environ.get("TARGET_BRANCH", "main"),
...
def slack_permalink(channel_id, ts):
    domain = os.environ.get("SLACK_WORKSPACE_DOMAIN", "").replace("https://", "").strip("/")
...
def cursor_prompt(summary, text):
    return "\n".join(
        [
            ...
            "Target branch: " + os.environ.get("TARGET_BRANCH", "main"),
            "",
...
def upsert_port_entity(entity):
    base_url = os.environ.get("PORT_BASE_URL", "https://api.getport.io").rstrip("/")
    ...
    blueprint = os.environ.get("PORT_BLUEPRINT_IDENTIFIER", "slack_use_case")
    post_json(
